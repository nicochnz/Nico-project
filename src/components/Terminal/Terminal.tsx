...
const TerminalPortfolio = () => {
	...
	const startHangman = () => {
		...
		setHistory([
			...history,
			"> hangman",
			"🎮 Dev Hangman started! Guess one letter at a time.",
		]);
	};
	...
	const handleHangmanGuess = (letter: string) => {
		...
		if (!hangmanWord.includes(letter)) {
			...
			if (newWrongGuesses >= maxWrongGuesses) {
				setHistory([
					...history,
					`> ${letter}`,
					`💀 You lost! The word was '${hangmanWord}'.`,
					"Oh, too bad! But at least you found out that Nicolas is looking for an internship! Hire him! ",
				]);
				setHangmanWord(null);
				return;
			}
		...
		const displayWord = hangmanWord
			.split("")
			.map((l) => (newGuessedLetters.includes(l) ? l : "_"))
			.join(" ");
		...
		if (!displayWord.includes("_")) {
			setHistory([
				...history,
				`> ${letter}`,
				`🎉 Congrats! You found the word '${hangmanWord}'! If you want more fun, hire Nicolas!`,
			]);
			setHangmanWord(null);
		}
	};
